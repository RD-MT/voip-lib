         ...
         public void onAvailable(final Network network) {
            NetworkManagerAbove24.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  ...
                  if (info == null) {
                     Log.i("[Platform Helper] [Network Manager 24] A network should be available but getNetworkInfo failed.");
                  } else {
            ...
            NetworkManagerAbove24.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  if (networkCapabilities == null) {
                     Log.i("[Platform Helper] [Network Manager 24] onCapabilitiesChanged called with null networkCapabilities, skipping...");
                  } else {
            ...
            NetworkManagerAbove24.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  if (linkProperties == null) {
                     Log.i("[Platform Helper] [Network Manager 24] onLinkPropertiesChanged called with null linkProperties, skipping...");
                  } else {
   ...
   public void updateDnsServers() {
      ...
      if (this.mConnectivityManager != null) {
         ...
         if (activeNetwork == null) {
            if (this.mNetworkAvailable == null) {
               Log.i("[Platform Helper] [Network Manager 24] Active network is null and no available network has been stored!");
            } else {
