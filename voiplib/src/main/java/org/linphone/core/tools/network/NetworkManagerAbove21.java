         ...
         public void onAvailable(final Network network) {
            NetworkManagerAbove21.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  ...
                  if (info == null) {
                     Log.i("[Platform Helper] [Network Manager 21] A network should be available but getNetworkInfo failed.");
                  } else {
            ...
            NetworkManagerAbove21.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  if (networkCapabilities == null) {
                     Log.i("[Platform Helper] [Network Manager 21] onCapabilitiesChanged called with null networkCapabilities, skipping...");
                  } else {
            ...
            NetworkManagerAbove21.this.mHelper.getHandler().post(new Runnable() {
               public void run() {
                  if (linkProperties == null) {
                     Log.i("[Platform Helper] [Network Manager 21] onLinkPropertiesChanged called with null linkProperties, skipping...");
                  } else {
